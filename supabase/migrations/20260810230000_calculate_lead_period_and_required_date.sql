-- Migration: Calculate per-line lead period and required date from Item Master Configuration
-- Description:
-- 1. Updates handle_auto_create_draft_pr_line() and handle_mr_submission_from_draft() triggers to look up lead_period_days from public.items (Item Master).
-- 2. Calculates expected/required date as: MR Submission Date + configured item lead_period_days.
-- 3. Sets lead_period_days, lead_period_date, and required_date for each purchase_requisition_lines row.

BEGIN;

-- 1. Replace handle_auto_create_draft_pr_line function
...
DECLARE
  ...
  v_mr_justification text;
  v_mr_submitted_at timestamptz;
  ...
  v_general_remarks text;

  -- Item Master Lead Period Details
  v_item_lead_days integer;
  v_calculated_required_date date;
BEGIN
  ...
  SELECT 
    mr_number, title, project_id, site_id, required_date, priority, site_block, raised_by, raised_by_name, raised_by_phone, justification, COALESCE(submitted_at, created_at, NOW())
  INTO 
    v_mr_number, v_mr_title, v_mr_project_id, v_mr_site_id, v_mr_required_date, v_mr_priority, v_mr_site_block, v_mr_raised_by, v_mr_raised_by_name, v_mr_raised_by_phone, v_mr_justification, v_mr_submitted_at
  ...
  -- Fetch Submitter / Site Engineer Details by ID
  ...
  -- Fallback by name
  IF v_profile_phone IS NULL AND v_mr_raised_by_name IS NOT NULL THEN
    SELECT name, COALESCE(phone, phone_number, mobile)
    INTO v_profile_name, v_profile_phone
    FROM public.profiles
    WHERE name ILIKE TRIM(v_mr_raised_by_name)
  ...
  v_general_remarks := NULLIF(TRIM(REGEXP_REPLACE(COALESCE(v_mr_justification, ''), '\s*•\s*Raised By:.*$', '', 'i')), '');

  -- Look up Lead Period from public.items table (Item Master)
  v_item_lead_days := NULL;
  IF NEW.item_id IS NOT NULL THEN
    SELECT lead_period_days INTO v_item_lead_days
    FROM public.items
    WHERE id = NEW.item_id;
  END IF;

  IF v_item_lead_days IS NULL AND NEW.item_code IS NOT NULL AND NEW.item_code <> '' THEN
    SELECT lead_period_days INTO v_item_lead_days
    FROM public.items
    WHERE item_code = NEW.item_code;
  END IF;

  IF v_item_lead_days IS NULL AND NEW.item_description IS NOT NULL AND NEW.item_description <> '' THEN
    SELECT lead_period_days INTO v_item_lead_days
    FROM public.items
    WHERE item_description ILIKE NEW.item_description
    ORDER BY created_at ASC
    LIMIT 1;
  END IF;

  -- Fallback to default of 3 days if not configured
  IF v_item_lead_days IS NULL THEN
    v_item_lead_days := 3;
  END IF;

  -- Calculate required/expected date: Submitted Date + Lead Period
  v_calculated_required_date := (v_mr_submitted_at::date + v_item_lead_days * INTERVAL '1 day')::date;
  ...
  ELSE
    ...
    SET 
      ...
      general_remarks = COALESCE(v_general_remarks, general_remarks),
      updated_at = NOW()
  ...
  -- Upsert corresponding PR line with lead time mappings and calculated required date
  IF EXISTS (
    ...
    SET
      ...
      lead_period_days = v_item_lead_days,
      lead_period_date = v_calculated_required_date,
      required_date = v_calculated_required_date,
      updated_at = NOW()
    WHERE purchase_requisition_id = v_pr_id AND material_request_line_id = NEW.id;
  ELSE
...
-- 2. Replace handle_mr_submission_from_draft function
...
BEGIN
  IF OLD.status = 'draft' AND (NEW.status = 'submitted' OR NEW.status = 'approved') THEN
    ...
    v_mr_submitted_at := COALESCE(NEW.submitted_at, NEW.created_at, NOW());
    ...
    -- Fallback by name
    IF v_profile_phone IS NULL AND NEW.raised_by_name IS NOT NULL THEN
      SELECT name, COALESCE(phone, phone_number, mobile)
      INTO v_profile_name, v_profile_phone
      FROM public.profiles
      WHERE name ILIKE TRIM(NEW.raised_by_name)
      ORDER BY created_at ASC
      LIMIT 1;
    END IF;
    ...
    v_general_remarks := NULLIF(TRIM(REGEXP_REPLACE(COALESCE(NEW.justification, ''), '\s*•\s*Raised By:.*$', '', 'i')), '');
    ...
    LOOP
      ...
      IF v_item_lead_days IS NULL AND v_line.item_code IS NOT NULL AND v_line.item_code <> '' THEN
        SELECT lead_period_days INTO v_item_lead_days
        FROM public.items
        WHERE item_code = v_line.item_code;
      END IF;

      IF v_item_lead_days IS NULL AND v_line.item_description IS NOT NULL AND v_line.item_description <> '' THEN
        SELECT lead_period_days INTO v_item_lead_days
        FROM public.items
        WHERE item_description ILIKE v_line.item_description
        ORDER BY created_at ASC
        LIMIT 1;
      END IF;
