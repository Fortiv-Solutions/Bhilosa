-- Migration: Map project and submitter details automatically from mobile application / Supabase to generated PR
-- Description:
-- 1. Updates handle_auto_create_draft_pr_line() to automatically fetch project address, city, pincode, location from projects table and map to delivery_address.
-- 2. Automatically fetches submitter profile name and registered contact phone number from profiles table and maps to site_contact_person and site_contact_number.
-- 3. Updates handle_mr_submission_from_draft() with identical data-driven mappings.
-- 4. Backfills existing PR headers with current project address and submitter contact details.

BEGIN;

-- 1. Replace handle_auto_create_draft_pr_line function with project and submitter mapping
...
BEGIN
  ...
  -- 2. Fetch Project Details from projects table
  ...
  -- 3. Fetch Submitter Details from profiles table
  ...
  v_site_contact_number := COALESCE(v_profile_phone, NULL);

  -- 4. If PR header does not exist, create it in 'under_verification' status
  ...
  -- 5. Purge any orphan PR lines for this PR
  ...
  -- 6. Upsert/Reconcile corresponding PR line for NEW.id
  IF EXISTS (
  ...
  -- 7. Mark MR line status as approved_for_pr
  ...
  -- 8. Ensure parent MR status is approved
...
-- 2. Replace handle_mr_submission_from_draft function with identical mappings
...
BEGIN
  IF OLD.status = 'draft' AND (NEW.status = 'submitted' OR NEW.status = 'approved') THEN
    ...
    -- Fetch Submitter Details from profiles table
...
-- 3. Backfill existing PR records with Project Address and Submitter details
UPDATE public.purchase_requisitions pr
SET 
  delivery_address = COALESCE(
    (
      SELECT COALESCE(p.project_address, p.communication_address, p.registered_address, p.location)
      FROM public.projects p WHERE p.id = pr.project_id
    ),
    pr.delivery_address
  ),
  site_contact_person = COALESCE(
    pr.site_contact_person,
    (SELECT prof.name FROM public.profiles prof WHERE prof.id = pr.prepared_by),
    pr.created_by_name
  ),
  site_contact_number = COALESCE(
    pr.site_contact_number,
    (SELECT prof.phone FROM public.profiles prof WHERE prof.id = pr.prepared_by)
  ),
  company_name = COALESCE(
    pr.company_name,
    (SELECT p.name FROM public.projects p WHERE p.id = pr.project_id)
  );
