-- =====================================================================
-- Migration: 20260805170500_bill_detail_mbi_fallback.sql
-- Purpose: Include master_budget_items (mbi.category_name) in COALESCE fallbacks
--          so linked master budget lines populate head_activity and category_name.
-- =====================================================================
...
BEGIN
  ...
  IF p_bill_source = 'material' THEN
    SELECT to_jsonb(vb) || jsonb_build_object(
             ...
             'allocation_name',    COALESCE(ba.allocation_name, bc.category_name, mbi.category_name),
             'category_name',      COALESCE(bc.category_name, mbi.category_name, ba.allocation_name),
  ...
  ELSE
    SELECT to_jsonb(sb) || jsonb_build_object(
             ...
             'head_activity',      COALESCE(bc.category_name, ba.allocation_name, mbi.category_name),
             'allocation_name',    COALESCE(ba.allocation_name, bc.category_name, mbi.category_name),
             'category_name',      COALESCE(bc.category_name, mbi.category_name, ba.allocation_name),
             'master_budget_item', mbi.item_description,
             'qc_status',          qc.status)
  ...
  SELECT COALESCE(jsonb_agg(
           jsonb_build_object(
             ...
             'allocation_name',  COALESCE(ba.allocation_name, (SELECT bc_sub.category_name FROM public.budget_categories bc_sub WHERE bc_sub.id = ba.category_id))
