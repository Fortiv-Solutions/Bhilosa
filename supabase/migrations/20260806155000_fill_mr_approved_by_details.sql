-- Update trigger functions to automatically populate reviewed_by, reviewed_by_name, and reviewed_at with the submitter's details
...
BEGIN
  ...
  SELECT 
    status::text, mr_number, title, project_id, site_id, required_date, priority::text, site_block, raised_by, raised_by_name
  INTO 
  ...
  IF v_pr_id IS NULL THEN
    ...
    -- Transition parent MR status to 'approved' and populate reviewer details with the submitter's details if it was submitted
    IF v_mr_status = 'submitted' THEN
      UPDATE public.material_requests 
      SET 
        status = 'approved',
        reviewed_by = raised_by,
        reviewed_by_name = raised_by_name,
        reviewed_at = NOW()
      WHERE id = NEW.material_request_id;
    END IF;
  END IF;
...
BEGIN
  ...
  IF OLD.status = 'draft' AND (NEW.status = 'submitted' OR NEW.status = 'approved') THEN
    ...
    -- Transition status to 'approved' and populate reviewer details with the submitter's details
    NEW.status := 'approved';
