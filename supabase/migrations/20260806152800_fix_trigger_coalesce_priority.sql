-- Recreate trigger functions with correct type casting to avoid enum validation issues
  ...
  IF OLD.status = 'draft' AND (NEW.status = 'submitted' OR NEW.status = 'approved') THEN
    ...
    IF v_pr_id IS NULL THEN
      ...
      INSERT INTO public.purchase_requisitions (
        ...
        CURRENT_DATE,
        NEW.required_date,
        COALESCE(NEW.priority::text, 'normal'), -- Explicitly cast enum to text to avoid type conversion error
        'material',
