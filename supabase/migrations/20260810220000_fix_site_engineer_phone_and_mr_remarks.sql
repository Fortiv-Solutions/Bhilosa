-- Migration: Fix Site Engineer phone lookup by name fallback and map MR justification to PR general_remarks
-- Description:
-- 1. Enhances handle_auto_create_draft_pr_line() and handle_mr_submission_from_draft() triggers to fall back to searching profiles by name if raised_by UUID is NULL.
-- 2. Automatically populates purchase_requisitions.general_remarks from material_requests.justification so MR remarks are visible in PR forms.
-- 3. Backfills site_contact_number and general_remarks for all existing PR records.
...
BEGIN
  ...
  SELECT 
    mr_number, title, project_id, site_id, required_date, priority, site_block, raised_by, raised_by_name, raised_by_phone, justification
  INTO 
    v_mr_number, v_mr_title, v_mr_project_id, v_mr_site_id, v_mr_required_date, v_mr_priority, v_mr_site_block, v_mr_raised_by, v_mr_raised_by_name, v_mr_raised_by_phone, v_mr_justification
  ...
  -- Fallback: If raised_by UUID was NULL or profile phone was NULL, search profile by name
...
-- 3. Backfill site_contact_number and general_remarks for all existing PRs
UPDATE public.purchase_requisitions pr
SET 
  site_contact_number = COALESCE(
    pr.site_contact_number,
    (SELECT COALESCE(p.phone, p.phone_number, p.mobile) FROM public.profiles p WHERE p.id = pr.prepared_by),
    (SELECT COALESCE(p.phone, p.phone_number, p.mobile) FROM public.profiles p WHERE p.name ILIKE pr.site_contact_person OR p.name ILIKE pr.created_by_name LIMIT 1)
  ),
  general_remarks = COALESCE(
    pr.general_remarks,
    (
      SELECT NULLIF(TRIM(REGEXP_REPLACE(mr.justification, '\s*•\s*Raised By:.*$', '', 'i')), '')
      FROM public.material_requests mr WHERE mr.id = pr.material_request_id
    )
  );
