...
export function TaskModule({ project, overviewData }: { project: any, overviewData: any }) {
  ...
  const handleTaskSubmit = (event: React.FormEvent) => {
    ...
    if (!normalizedName || !assignee || !taskStartDate || !taskEndDate) {
      setTaskFormError('Complete all task fields before assigning.');
      return;
    }
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-start justify-between">
            ...
            <div className="relative group/dropdown">
              ...
              <div className="absolute right-0 top-full mt-1 w-32 rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-xl opacity-0 invisible group-hover/dropdown:opacity-100 group-hover/dropdown:visible transition-all z-10 p-1">
                <select
                  value={taskStatus}
                  onChange={(e) => updateTask(project.id, task.id, { status: e.target.value as any })}
                  className="w-full appearance-none rounded-lg p-2 text-xs font-medium text-gray-700 dark:text-gray-300 outline-none hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer"
                >
            ...
            <form onSubmit={handleTaskSubmit} className="mb-6 relative overflow-hidden rounded-2xl border border-gray-100 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 p-5 lg:p-6 shadow-sm backdrop-blur-xl">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <label className="block">
                  <span className="mb-1.5 block text-[10px] font-bold uppercase tracking-wider text-gray-500">Assign to</span>
