...
export function BillsForm({ bill, approvedGrns = [], vendorOptions = [], onSubmit, onPrint, onCancel }: BillsFormProps) {
  ...
  const [form, setForm] = useState<FullBillsFormState>(() => {
    ...
    return {
      ...
      project_name: (raw.projects as { name?: string })?.name || (raw.project_name as string) || bill.project_name || 'Silvassa Unit I - Polyester Division',
      company_name: (raw.company_name as string) || bill.company_name || 'Bhilosa Industries Private Limited',
      supplier_name: (raw.vendors as { display_name?: string; legal_name?: string })?.display_name || (raw.vendors as { legal_name?: string })?.legal_name || bill.vendor_name || 'Reliance Polyester Raw Materials Ltd',
