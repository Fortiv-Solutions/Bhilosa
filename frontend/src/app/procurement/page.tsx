      ...
      {activeTab === 'requests' && (
        <Panel title="Material Requests" icon={ClipboardList}>
          <MaterialRequestWorkQueue
            ...
            onPrintMr={(targetMr) => {
              const mr = typeof targetMr === 'string' ? data.materialRequests.find((m) => m.id === targetMr) : targetMr;
              if (!mr) return;
              void (async () => {
                setError(null);
                setMessage(null);
                try {
                  const pdfBlob = await generateMaterialRequestPdfBlob(mr);
                  downloadMaterialRequestPdfFile(mr, pdfBlob);
                  setMessage(`Material Request PDF downloaded: ${mr.mr_number}.pdf`);
                } catch (err: any) {
      ...
      {activeTab === 'rfq' && (
        <Panel title="RFQ, Quotations, and Vendor Finalization" icon={UsersRound}>
          <RFQWorkspace
            ...
            onGeneratePo={(pr, quotation, selection) => {
              runAction(`Auto-Draft Purchase Order created for ${pr.pr_number}.`, async () => {
                const vendorId = quotation?.vendor_id || selection?.selected_vendor_id || data.vendors[0]?.id || '';
                const lines = (pr.purchase_requisition_lines || []).map((l) => ({
                  item_id: l.item_id ?? null,
                  item_description: l.item_description,
                  quantity: Number(l.quantity || 1),
                  unit_rate: Number(l.estimated_rate || 350),
                  tax_rate: 18,
                  line_total: Number(l.quantity || 1) * Number(l.estimated_rate || 350),
                }));

                const isUuid = (id: string | null | undefined): boolean =>
                  !!id && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id);
                const rawSelId = selection?.id || null;
                const safeSelId = isUuid(rawSelId) ? rawSelId : null;

                return generatePurchaseOrder({
                  purchaseRequisitionId: pr.id,
                  vendorId,
                  vendorSelectionId: safeSelId,
                  deliveryDate: pr.required_date || new Date().toISOString().split('T')[0],
                  deliveryLocation: pr.delivery_address || 'Project site store',
...
17. Price Basis - DAP at Site, Freight included`,
            ...
            onPrintRfq={(rfqId) => {
              ...
              if (rfq) {
                ...
                handleReportPdf('RFQ', () => generateRfqPdf(rfqId));
              }
            }}
          />
        </Panel>
      ...
      {activeTab === 'billing' && (
        <Panel title="Vendor Bills & 3-Way Matching" icon={ReceiptIndianRupee}>
          <BillsWorkspace
            ...
            activeRole={activeRole as 'UPPER_MANAGEMENT' | 'PROJECT_MANAGER' | 'PR_TEAM'}
            onPrintBill={(billId) => {
              const bill = data.vendorBills.find((b) => b.id === billId);
              if (bill) {
                printPurchaseBillReport(bill);
              } else {
                handleReportPdf('Purchase Bill', () => generatePurchaseBillPdf(billId));
              }
            }}
            onRefresh={refresh}
          />
